...
__host__ void run_afl_compress_cpu(const unsigned int bit_length, T *data,
	...
	for (tid = 0, bid = 0; bid <= block_number; tid++) {
		...
		afl_compress_base_gpu<T, CWARP_SIZE>(bit_length, data_id, cdata_id,
				data, compressed_data, length, comprLength);
	}
}
...
__host__ void run_afl_compress_gpu(const unsigned int bit_length, T *data,
		T *compressed_data, unsigned long length, unsigned long comprLength) {
...
template<typename T, char CWARP_SIZE>
__global__ void afl_compress_gpu(const unsigned int bit_length, T *data,
		T *compressed_data, unsigned long length, unsigned long comprLength) {
	const unsigned int warp_lane = (threadIdx.x % CWARP_SIZE);
	const unsigned long data_block = blockIdx.x * blockDim.x + threadIdx.x
			- warp_lane;
	...
	afl_compress_base_gpu<T, CWARP_SIZE>(bit_length, data_id, cdata_id, data,
			compressed_data, length, comprLength);
}

template<typename T, char CWARP_SIZE>
__global__ void afl_decompress_gpu(const unsigned int bit_length,
		T *compressed_data, T * decompress_data, unsigned long length) {
	const unsigned int warp_lane = (threadIdx.x % CWARP_SIZE);
	const unsigned long data_block = blockIdx.x * blockDim.x + threadIdx.x
			- warp_lane;
...
template<typename T, char CWARP_SIZE>
__device__ __host__ void afl_compress_base_gpu(const unsigned int bit_length,
		unsigned long data_id, unsigned long comp_data_id, T *data,
		T *compressed_data, unsigned long length, unsigned long comprLength) {
	T v1, value = 0;
...
template<typename T, char CWARP_SIZE>
__device__ __host__ void afl_decompress_base_gpu(const unsigned int bit_length,
		unsigned long comp_data_id, unsigned long data_id, T *compressed_data,
		T *data, unsigned long length) {
