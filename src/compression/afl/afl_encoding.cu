...
SharedCudaPtrVector<char> AflEncoding::Encode(SharedCudaPtr<T> data) {
	...
	LOG4CPLUS_INFO_FMT(_logger, "AFL encoding START: data size = %lu",
			data->size());

	LOG4CPLUS_TRACE_FMT(_logger, "AFL data to encode: %s",
...
__global__ void _splitFloatKernel(float* data, size_t size, int* mantissa,
...
SharedCudaPtrVector<char> AflEncoding::Encode(SharedCudaPtr<float> data) {
	CUDA_ASSERT_RETURN(cudaGetLastError());
	LOG4CPLUS_INFO_FMT(_logger, "AFL (FLOAT) encoding START: data size = %lu",
			data->size());
	...
	cudaLaunch(this->_policy, _splitFloatKernel, data->get(), data->size(),
...
__global__ void _composeFloatKernel(int* mantissa, int* exponent, int* sign,
...
SharedCudaPtr<float> AflEncoding::Decode(SharedCudaPtrVector<char> input) {
	...
	CUDA_CALL(
			cudaMemcpy(&compressedMantissaSize, metadata->get(), sizeof(size_t), CPY_DTH));
	...
	// decode mantissa
	auto mantissaDecoded = DecodeAfl<int>((int*) (data->get() + offset),
	...
	// decode exponent
	auto exponentDecoded = DecodeAfl<int>((int*) (data->get() + offset),
	...
	cudaLaunch(this->_policy, _composeFloatKernel, mantissaDecoded->get(),
...
SharedCudaPtrVector<char> AflEncoding::EncodeDouble(
		SharedCudaPtr<double> data) {
	return SharedCudaPtrVector<char>();
}
SharedCudaPtr<double> AflEncoding::DecodeDouble(
